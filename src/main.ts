...
const render = (): void => {
  ...
  const active = Boolean(playState && activeUnit?.side === 'ally' && playState.result === 'playing' && !autoMover.active);
  ...
  editorUnitOptions.classList.toggle('hidden', Boolean(playState));
...
const setDirection = (direction: Direction): void => { const unit = selectedUnit(); if (!playState || !unit || autoMover.active || currentUnit(playState)?.id !== unit.id || unit.side !== 'ally') return; changeDirection(unit, direction); playState.phase = 'move'; playState.message = `向きを${directionLabels[direction]}に変更しました。`; };
const animateMovement = (): void => {
  const unit = selectedUnit();
  if (!playState || !unit || !autoMover.active) return;
  autoMover.advance(unit); render();
  if (autoMover.active) window.setTimeout(animateMovement, 120);
  else { playState.message = '移動しました。続けて範囲内を移動できます。'; render(); }
};
const playTap = (x: number, y: number): void => {
  ...
  if (playState.phase === 'skill' && playState.selectedSkillId) {
    const result = useUnitSkill(playState, selected.id, playState.selectedSkillId, { x, y });
    if (result.success) { playState.selectedSkillId = undefined; if (playState.result === 'playing') finishAction(); else render(); }
    else { playState.message = 'そのマスにはスキルを使用できません。'; }
    return;
  }
...
app.querySelector('#edit')!.addEventListener('click', () => { autoMover.cancel(); playState = null; render(); });
app.querySelector('#play')!.addEventListener('click', () => {
  autoMover.cancel(); playState = { stage: cloneStage(editStage), turn: 'ally', round: 1, actionOrder: [], currentActionIndex: 0, selectedUnitId: null, phase: 'select', origin: null, result: 'playing', message: '' };
...
app.querySelectorAll<HTMLButtonElement>('.skill-button').forEach((button) => button.addEventListener('click', () => {
  if (!playState) return; const unit = currentUnit(playState); const skill = unit && skillsForUnit(unit)[Number(button.dataset.skillIndex)]; if (!unit || !skill) return;
  playState.selectedSkillId = skill.id; playState.phase = 'skill'; playState.message = `${skill.name}の対象マスを選択してください。`; render();
}));
...
app.querySelector('#cancel')!.addEventListener('click', () => { if (playState && (playState.phase === 'direction' || playState.phase === 'skill')) { playState.phase = 'move'; playState.selectedSkillId = undefined; playState.message = '自由移動を続けられます。'; render(); } });
