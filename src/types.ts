...
export interface PlayState { stage: StageData; turn: Turn; selectedUnitId: string | null; phase: PlayPhase; origin: TurnOrigin | null; round: number; actionOrder: string[]; actionIndex: number; result: Result; message: string }
export const defaultUnit = (side: UnitSide, x: number, y: number): Unit => ({ id: crypto.randomUUID(), side, x, y, hp: 10, maxHp: 10, attack: 3, move: 3, speed: 10, direction: side === 'ally' ? 'up' : 'down', acted: false });
