...
export type SkillId = 'power-slash' | 'heavy-break' | 'shield-bash' | 'rush' | 'guard';
...
export interface Unit { id: string; name?: string; skillId?: SkillId; side: UnitSide; x: number; y: number; hp: number; maxHp: number; attack: number; defense: number; currentMp: number; maxMp: number; guarding: boolean; move: number; speed: number; direction: Direction; acted: boolean }
...
export const defaultUnit = (side: UnitSide, x: number, y: number): Unit => ({ id: crypto.randomUUID(), side, x, y, hp: 50, maxHp: 50, attack: 20, defense: DEFAULT_DEFENSE, currentMp: DEFAULT_CURRENT_MP, maxMp: DEFAULT_MAX_MP, guarding: false, move: 3, speed: DEFAULT_SPEED, direction: side === 'ally' ? 'up' : 'down', acted: false });

export const PLAYER_PRESETS = {
  Balance: { hp: 50, attack: 20, defense: 20, currentMp: 30, maxMp: 50, skillId: 'power-slash' },
  Attacker: { hp: 42, attack: 26, defense: 14, currentMp: 30, maxMp: 40, skillId: 'heavy-break' },
  Tank: { hp: 70, attack: 14, defense: 32, currentMp: 20, maxMp: 40, skillId: 'shield-bash' },
  Assault: { hp: 47, attack: 24, defense: 18, currentMp: 40, maxMp: 50, skillId: 'rush' },
  Defender: { hp: 60, attack: 18, defense: 26, currentMp: 30, maxMp: 60, skillId: 'guard' },
} as const;
...
export const playerUnit = (name: PlayerPresetName, x: number, y: number): Unit => {
  ...
  return { ...defaultUnit('ally', x, y), id: `ally-${name.toLowerCase()}`, name, hp: preset.hp, maxHp: preset.hp, attack: preset.attack, defense: preset.defense, currentMp: preset.currentMp, maxMp: preset.maxMp, skillId: preset.skillId };
};
