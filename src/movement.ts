...
const movementSearch = (stage: StageData, unit: Unit): { came: Map<string, Position | null>; costs: Map<string, number> } => {
  ...
  while (queue.length > 0) {
    ...
    const current = queue.shift()!; const currentCost = costs.get(key(current))!;
    ...
    for (const edge of travelEdges(stage, current, unit.id)) {
      ...
      if (nextCost > unit.move || nextCost >= (costs.get(nextKey) ?? Infinity)) continue;
      costs.set(nextKey, nextCost); came.set(nextKey, current); queue.push(edge.position);
    }
  }
  return { came, costs };
};
export const movementMap = (stage: StageData, unit: Unit): Map<string, Position | null> => movementSearch(stage, unit).came;
export const movementCosts = (stage: StageData, unit: Unit): Map<string, number> => movementSearch(stage, unit).costs;
...
export const flickDirection = (dx: number, dy: number, threshold = 20): Direction | null => {
